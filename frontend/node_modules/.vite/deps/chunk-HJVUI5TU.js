...
function useDelayGroup(context, options = {
  ...
  const isUnmountedRef = React2.useRef(false);
  useIsoLayoutEffect(() => {
...
// node_modules/@base-ui/react/utils/popups/inlineRect.mjs
...
function getInlineReferenceRect(reference, placement, coords) {
  ...
  const x = coords == null ? void 0 : coords.x;
  const y = coords == null ? void 0 : coords.y;
  const side = placement[0];
  if ((coords == null ? void 0 : coords.lineIndex) != null && lines[coords.lineIndex]) {
...
function createInlineMiddleware(coordsRef) {
  return {
    ...
    async fn(state) {
      ...
      if (typeof (reference == null ? void 0 : reference.getClientRects) !== "function") {
        return {};
      }
      const contextElement = getContextElement(reference);
      const coords = coordsRef.current;
      const currentCoords = (coords == null ? void 0 : coords.element) === reference || (coords == null ? void 0 : coords.element) === contextElement ? coords : void 0;
...
function useForcedRerendering() {
  const [, setState] = React14.useState({});
...
function applyPopupOpenChange(store, nextOpen, eventDetails, options = {}) {
  ...
  (_b = (_a = store.context).onOpenChange) == null ? void 0 : _b.call(_a, nextOpen, eventDetails);
  if (eventDetails.isCanceled) {
    return;
  }
...
function useFloating2(options = {}) {
  ...
  const [localDomReference, setLocalDomReference] = React18.useState(void 0);
  const [localFloatingElement, setLocalFloatingElement] = React18.useState(void 0);
  const domReferenceRef = React18.useRef(null);
  const tree = useFloatingTree(externalTree);
  const storeElements = React18.useMemo(() => ({
...
function useHoverReferenceInteraction(context, props = {}) {
  ...
  const tree = useFloatingTree(externalTree);
  const instance = useHoverInteractionSharedState(store);
  const isHoverCloseActiveRef = React21.useRef(false);
  ...
  React21.useEffect(() => cleanupMouseMoveHandler, [cleanupMouseMoveHandler]);
  React21.useEffect(() => {
    if (!enabled) {
      return void 0;
    }
...
function useListNavigation(context, props) {
  ...
  const parentId = useFloatingParentNodeId();
  const tree = useFloatingTree(externalTree);
  const focusItemOnOpenRef = React23.useRef(focusItemOnOpen);
  const indexRef = React23.useRef(selectedIndex ?? -1);
  const keyRef = React23.useRef(null);
  const isPointerModalityRef = React23.useRef(true);
  const onNavigate = useStableCallback((event) => {
    onNavigateProp(indexRef.current === -1 ? null : indexRef.current, event);
  });
  const previousMountedRef = React23.useRef(!!floatingElement);
  const previousOpenRef = React23.useRef(open);
  const forceSyncFocusRef = React23.useRef(false);
  const forceScrollIntoViewRef = React23.useRef(false);
  const cancelQueuedFocusRef = React23.useRef(null);
  ...
  const ariaActiveDescendantProp = React23.useMemo(() => {
...
function useTypeahead(context, props) {
  ...
  const sharedProps = React24.useMemo(() => ({
    onKeyDown,
    onBlur
  }), [onKeyDown, onBlur]);
...
use-sync-external-store/cjs/use-sync-external-store-shim/with-selector.development.js:
   ...
   *)
*/
//# sourceMappingURL=chunk-HJVUI5TU.js.map
